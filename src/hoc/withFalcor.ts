...
export const WithFalcor = <Props, Fragment = TypedFragment>(model: Model, graphChange$: Observable<undefined>, options: Options = {}) => {
  const connectedModel = connect(model, graphChange$, options)

  return (
    paths: PathSet[] | Error | null | ((props: Props) => PathSet[] | Error | null),
  ) => mapPropsStream<ChildProps<Fragment> & Props, Props>((props$) => combineLatest(
    props$,
    props$.pipe(
